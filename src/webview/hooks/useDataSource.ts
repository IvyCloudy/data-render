...
function generateCacheKey(url: string, query: Record<string, unknown> | undefined, transformPath?: string): string {
    const qs = query ? Object.keys(query).sort().map((k) => `${k}=${JSON.stringify(query[k])}`).join('&') : '';
    const pathSuffix = transformPath ? `#path=${transformPath}` : '';
    return `${url}?${qs}${pathSuffix}`;
}
...
function setCache(key: string, options: Array<{
    label: string;
    value: unknown;
    disabled?: boolean
}>, value: unknown, ttl: number): void {
...
export function useDataSource(
    ...
    const fetchData = useCallback(async () => {
        ...
        const cacheKey = generateCacheKey(fullUrl, interpolatedQuery, dataSource.transform?.path);
